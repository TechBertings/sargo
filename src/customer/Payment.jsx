...
const Payment = ({ reservationData, onBack, onSuccess }) => {
 ...
 const formatTime12H = (time24) => {
  ...
  const handleSubmit = async () => {
    // ============= VALIDATION =============
    if (formData.payment_method === "GCash") {
      // ✅ NEW: Check reference number length FIRST
      ...
      // ✅ NEW: Check if exactly 13 digits
      if (formData.referenceNumber.length !== 13) {
        Swal.fire({
          icon: "error",
          title: "Invalid Reference Number",
          html: `
            <p class="text-gray-700 mb-2">GCash reference number must be exactly <strong>13 digits</strong></p>
            <p class="text-sm text-gray-600">Current: <strong class="text-red-600">${formData.referenceNumber.length} digit${formData.referenceNumber.length !== 1 ? 's' : ''}</strong></p>
            <p class="text-sm text-gray-600">Required: <strong class="text-green-600">13 digits</strong></p>
          `,
...
const reservationInserts = reservations.map((reservation, idx) => {
  ...
  const record = {
    ...
    duration: parseFloat(reservation.duration.hours),
    status: "pending",
    ...
    // ✅ FIX: Only store proof_of_payment and reference_no for GCash
    proof_of_payment:
      formData.payment_method === "GCash" ? formData.proofOfPayment : null,
    // ✅ FIX: Only store reference_no if GCash AND has reference number
  ...
  console.log(`📝 [RECORD ${idx + 1}]`, {
    ...
    reference_no: record.reference_no || "NULL", // ✅ Should be NULL for Cash
  ...
  return record;
      ...
      // ✅ GET CUSTOMER NAME from userSession (more reliable)
      const customerName = userSession?.full_name || userSession?.email || currentUser?.full_name || currentUser?.email || "Customer";

      console.log("👤 Customer Name:", customerName);
      console.log("📧 Account ID:", accountId);

      // ✅ CUSTOMER NOTIFICATION (Simple confirmation)
      const { error: customerNotifError } = await supabase.from("notification").insert({
        account_id: parseInt(accountId),
        reservation_no: reservationNo,
        message: `${customerName} submitted a new reservation ${reservationNo} pending approval`,
        is_read: false,
        created_at: new Date().toISOString(),
      });

      if (customerNotifError) {
        console.error("⚠️ [STEP 4 WARNING] Customer notification insert failed:", customerNotifError);
      } else {
        console.log("✅ Customer notification created");
      }

      // ✅ MANAGER/FRONTDESK NOTIFICATION (Detailed with customer name)
      const { error: managerNotifError } = await supabase.from("notification").insert({
      ...
      if (managerNotifError) {
        console.error("⚠️ [STEP 4 WARNING] Manager notification insert failed:", managerNotifError);
      } else {
        console.log("✅ Manager notification created");
      }

      console.log("✅ [STEP 4 SUCCESS] Notifications created");
 ...
 const formatTime12H = (time24) => {
  ...
  <div>
    <span style="color:#999;">🕐 Time Start:</span>
  ...
  <!-- TIME END -->
  <div>
    <span style="color:#999;">⏰ Time End:</span>
    <div style="font-weight:600; color:#333;">
      ${formatTime12H(res.timeEnd)}
    </div>
  </div>
...
<div style="margin-top:8px;">
  ...
  <div style="font-weight:600; color:#333;">
    ${res.duration.hours} hour${res.duration.hours > 1 ? "s" : ""}
  </div>
</div>
  `
        )
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-5">
      <div className="max-w-3xl mx-auto pb-24">
        {/* Header */}
        <div className="bg-white p-8 rounded-xl mb-5 shadow-md">
  ...
  >
    {formatTime12H(reservation.time)} - {formatTime12H(reservation.timeEnd)}
  </p>
</div>
                  <div>
                    <p
              ...
              >
                <div style={{ marginBottom: "15px" }}>
                  ...
                  >
                    ...
                    <li><strong>No online cash payment</strong> - This booking requires physical presence</li>
              ...
              <div style={{ marginBottom: "20px" }}>
                ...
                >
                  ...
                  >
                    ...
                    Scan any QR code above to send your payment, then enter the{" "}
                    <strong>reference number</strong> and upload the{" "}
                    <strong>screenshot</strong> below.
                  </div>
                </div>
              </div>
              ...
              <div className="mb-5 relative">
                <label className="flex items-center gap-2 mb-2 text-sm font-semibold text-gray-700">
                  ...
                  <div className="relative group">
                    <span className="flex items-center justify-center w-4 h-4 text-xs text-white bg-blue-500 rounded-full cursor-help">
                      ?
                    </span>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 border-2 rounded-lg text-sm font-mono transition-all ${formData.referenceNumber.length === 13
                ...
                <div className="mt-2 text-sm">
                  <span className={`font-semibold ${formData.referenceNumber.length === 13
                    ? 'text-green-600'
                    : formData.referenceNumber.length > 0
                      ? 'text-yellow-600'
                      : 'text-gray-500'
                    }`}>
                    {formData.referenceNumber.length}/13 digits
                  </span>
                  {formData.referenceNumber.length > 0 && formData.referenceNumber.length < 13 && (
                    <span className="ml-2 text-red-600">
                      • {13 - formData.referenceNumber.length} more digit{13 - formData.referenceNumber.length > 1 ? 's' : ''} required
                    </span>
                  )}
